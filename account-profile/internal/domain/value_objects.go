...
const (
	...
	SettingsActivitySearchUnit  = "activity.search_unit"
)
...
func (s *Setting) FloatValue() (float64, error) {
	if s.valueType != SettingTypeFloat {
...
func validateSettingValue(value string, settingType SettingType) error {
	...
	case SettingTypeFloat:
		if _, err := strconv.ParseFloat(value, 64); err != nil {
			return fmt.Errorf("%w: invalid float format", ErrInvalidSettingsValue)
		}
	case SettingTypeString:
		if value == "" {
			return fmt.Errorf("%w: empty string is not allowed", ErrInvalidSettingsValue)
		}
	case SettingTypeInt:
		if _, err := strconv.Atoi(value); err != nil {
			return fmt.Errorf("%w: invalid int format", ErrInvalidSettingsValue)
		}
	default:
		return ErrInvalidSettingsValue
	}
